...
When I heard Karpathy say this, I wanted to find out how. How does one person ship like a team of twenty? The revolution is here. A single builder with the right tooling can move faster than a traditional team.
...
| `/pair-agent` | **Multi-Agent Coordinator** | Share your browser with any AI agent. One command, one paste, connected. Works with Hermes, Codex, Cursor, or anything that can curl. Each agent gets its own tab. Auto-launches headed mode so you watch everything. Auto-starts ngrok tunnel for remote agents. Scoped tokens, tab isolation, rate limiting, activity attribution. |
...
| `/ios-qa` | **iOS Live-Device QA (v1.43.0.0+)** — drive a real iPhone over USB CoreDevice via an embedded `StateServer` in the app. Read Swift source, codegen typed `@Observable` accessors, run the agent loop. Capability-tier allowlist (observe/interact/mutate/restore), per-device session lock, audit log. The optional `--tailnet` remote-exposure path is gated off on Thanx-fork installs (`gstack-thanx-fork-check`); local USB QA works normally. |
...
/guard, /unfreeze, /gstack-upgrade, /upstream-sync, /learn.
```
