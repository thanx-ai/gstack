...
StateServer in the app under test. The optional Tailscale `--tailnet`
remote-exposure path is gated off on Thanx-fork installs; local USB QA
works normally.
...
```bash
# Thanx fork: no remote VERSION poll. Updates land via /gstack-upgrade
...
```
       ...
       └──────────────────────┘                           └──────────────────┘
                ▲
                │ Tailscale (optional, --tailnet — gated off on Thanx fork)
                │
       ┌──────────────────────┐
       │ Remote agent         │
       │ (Codex, etc.)        │
...
tokens (default 1h) for remote agents. On Thanx-fork installs the `--tailnet`
listener is refused by `gstack-thanx-fork-check`; local USB QA is unaffected.
