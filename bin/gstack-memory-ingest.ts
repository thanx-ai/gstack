...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // remote without an actual clean scan. (Thanx fork keeps this scan
    // unconditional rather than opt-in: defense-in-depth even though gbrain
    // sync is fork-gated and dormant on Thanx installs.)
