...
describe('SKILL.md size budget regression (gate, free)', () => {
  ...
  test('no skill shrinks past 80% of v1.47.0.0 baseline (catches accidental body strip)', () => {
    ...
    const SECTIONS_EXTRACTED = new Set<string>(['ship', 'plan-ceo-review', 'office-hours', 'plan-eng-review', 'plan-design-review', 'plan-devex-review',
      // Thanx fork: gstack-upgrade was rewritten smaller than upstream's v1.47
      // baseline — the auto-update network poll + inline UPGRADE_AVAILABLE flow
      // were removed (fork posture: no remote VERSION poll). The shrink is
      // intentional hardening, not an accidental body strip.
      'gstack-upgrade']);
