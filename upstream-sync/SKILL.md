...
- Any change that re-arms `bin/gstack-telemetry-sync` (currently a stub).
  Restoring its original body would re-enable POSTs to the upstream
  Supabase project. Same threat shape as the update-check re-arm.
- Any change that recreates `supabase/config.sh` or any file under
  `supabase/`. Those files carried the upstream Supabase URL and were
  deleted as defense-in-depth alongside the telemetry stub.
- Any change to `scripts/resolvers/preamble/generate-telemetry-prompt.ts`
  that makes it return non-empty content. The empty return is what
  removes the AskUserQuestion that would prompt users to opt into
  remote telemetry.
- Any change that removes the Step 0 Thanx-fork guard from
  `setup-deploy/SKILL.md.tmpl`, `setup-gbrain/SKILL.md.tmpl`, or
  `pair-agent/SKILL.md.tmpl` (the remote/ngrok branch). Those guards
  prevent the policy-violating skill paths from running on Thanx
  installs.
- Any change to `bin/gstack-thanx-fork-check` that softens the origin
  match (e.g. broader globs) or alters the fail-closed defaults
  (script-path lookup failure / empty origin must continue to exit 0).
- Any change that removes the Thanx-fork guard from
  `bin/gstack-community-dashboard` or `bin/gstack-security-dashboard`.
...
- Any change to `/upstream-sync` itself (this skill) that loosens the
  review steps.
- Any change under `.github/workflows/` that reintroduces
  `ubicloud-standard-8` runners (the fork has no self-hosted Ubicloud
  fleet — those jobs queue forever and never gate), or that makes the
  paid eval suite (`evals.yml` / "E2E Evals") or the Ubicloud Docker
  build (`ci-image.yml` / "Build CI Image") gate PRs again. The fork
  keeps those `workflow_dispatch`-only and runs everything else on
...
### Test reconciliation (expected on every sync)

`bun test` will NOT be fully green after a sync, and that is expected — do
not panic-revert. Two distinct classes of failure show up:

1. **Upstream tests that assert the exfil code this fork removed.** Upstream
   ships tests checking the live telemetry/update/supabase paths (e.g. a
   `gstack-telemetry-sync` repo-identity sed test, a `supabase/verify-rls.sh`
   mktemp test). They fail here because the fork deleted their subject.
   **Delete these tests — do not make them green.** Making them pass would
   require re-arming the exact exfil this fork exists to close. The stubs stay
   covered by `test/telemetry.test.ts` and `test/skill-validation.test.ts`.
   Record each deletion in the sync log.
2. **Fork-only registration drift.** Fork-only skills/files need adding to
   upstream's test registries: register `/upstream-sync` in
   `test/skill-coverage-matrix.ts`; exempt the fork-rewritten (smaller)
   `gstack-upgrade` in `test/skill-size-budget.test.ts`. Resolve generated
   `SKILL.md` conflicts via `bun run gen:skill-docs --host all`, never by hand;
   golden fixtures are exact copies of the regenerated host ship SKILL.md.

**Blame protocol — diff against a baseline before attributing any failure to
the sync.** A clean fork checkout's `bun test` is already red (~24 failures in
a typical dev box: gbrain CLI, swift, gitleaks, codex toolchains absent). Run
`bun test` on `origin/main` in a throwaway worktree, diff the failure sets, and
only act on failures the sync actually introduced. In CI the GitHub-hosted
`gate` check is the real signal.

If failures remain that are NOT environment/upstream-inherited and NOT the two
classes above, **stop** — do not push, do not open a PR. Surface them and let
the user decide whether to fix-on-this-branch or abort the sync.

Merge the sync PR with `--merge` (a merge commit), not squash, so the fork's
`main` retains upstream's commit history. That keeps the merge-base correct so
the next `/upstream-sync` only reviews genuinely-new upstream commits.
